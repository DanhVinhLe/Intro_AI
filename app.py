...
from algorithms import a_star, bfs, dfs, dijkstra, greedy, nearest_node, SPFA
...
class MapApp:
    ...
    def show_all_nodes(self):
        ...
        self.ax.clear()
        self.ax.imshow(self.background_image, extent=[105.83469, 105.85143, 21.03979, 21.05118], aspect='auto', zorder=0)
    ...
    def calculate_route(self):
        ...
        if self.selected_algorithm == "DFS" or self.selected_algorithm == "BFS":
            start,_ = nearest_node.nearest_node(graph, [self.selected_points[0][0], self.selected_points[0][1]], k=1, heuristic= nearest_node.heuristic)
            end,_ = nearest_node.nearest_node(graph, [self.selected_points[1][0], self.selected_points[1][1]], k=1, heuristic= nearest_node.heuristic)
            self.start_node = start[0]
            self.end_node = end[0]
            if self.selected_algorithm == "DFS":
                path = dfs.dfs(graph, self.start_node, self.end_node)
            else:
                path, _ = bfs.bfs(graph, self.start_node, self.end_node)
            if path is None:
            ...
            near_start, distance_start = nearest_node.nearest_node(graph, [self.selected_points[0][0], self.selected_points[0][1]], k= 3, heuristic= nearest_node.heuristic)
            near_end, distance_end = nearest_node.nearest_node(graph, [self.selected_points[1][0], self.selected_points[1][1]], k= 3, heuristic= nearest_node.heuristic)
            ...
            if self.selected_algorithm == "A*":
                ...
                for id1, i in enumerate(near_start):
                    for id2, j in enumerate(near_end):
                        path, dis = dijkstra.dijkstra(graph, i, j)
                        if path is None:
                            continue
                        dis += (distance_start[id1] + distance_end[id2])
                        if dis < dis_min:
                ...
                for id1, i in enumerate(near_start):
                    for id2, j in enumerate(near_end):
                        path, dis = greedy.greedy_best_first_search(graph, i, j, heuristic= greedy.heuristic)
                        if path is None:
                            continue
                        dis += (distance_start[id1] + distance_end[id2])
                        if dis < dis_min:
                ...
                if not check:
                    messagebox.showwarning("No Path", "No path found between the selected points.")
                    return
            elif self.selected_algorithm == "SPFA":
                for id1, i in enumerate(near_start):
                    for id2, j in enumerate(near_end):
                        path, dis = SPFA.spfa(graph, i, j)
                        if path is None:
                            continue
                        dis += (distance_start[id1] + distance_end[id2])
                        if dis < dis_min:
